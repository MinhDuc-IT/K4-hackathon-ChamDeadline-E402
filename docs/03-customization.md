...
## 2. Cách thay đổi ID Tag Admin
Khi Bot gặp câu hỏi khó không có trong kho dữ liệu, nó sẽ tự động nhận diện "biết-mình-không-biết" và gọi Admin vào trả lời.

Hiện tại nó đang in ra chữ `@Admin` dạng text thường. Để nó có thể **nhắc tên (Ping)** Role Admin thật sự trên Discord:
1. Bật tính năng Developer Mode trên Discord (User Settings > Advanced > Developer Mode).
2. Vào Server, vào phần Roles (Vai trò), chuột phải vào Role của Admin/TA và chọn **Copy Role ID** (ví dụ: `123456789`).
...
```
Thay thế `@Admin` bằng cú pháp `<@&123456789>` (thay 123456789 bằng ID bạn vừa copy).
